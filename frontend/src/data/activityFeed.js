...
export function getActivityFeed(student) {
  ...
  for (const link of paymentLinks) {
    const entry = {
      title: link.generatedBy ? `Payment Link Generated by ${link.generatedBy}` : "Payment Link Generated",
  ...
  for (const p of payments) {
    ...
    const entry = {
      title: p.addedBy ? `Payment Added by ${p.addedBy}` : "Payment Added",
  ...
  if (s.orderPunched && s.orderPunchedAt) {
    allItems.push({
      title: s.orderPunchedBy ? `Order Punched by ${s.orderPunchedBy}` : "Order Punched",
  ...
  if (s.status === "Enrolled" && s.enrolledAt) {
    allItems.push({
      title: s.enrolledBy ? `Enrolled by ${s.enrolledBy}` : "Enrolled",
  ...
  if (s.status === "Cancelled" && s.cancelledAt) {
    allItems.push({
      title: s.cancelledBy ? `Student Registration Cancelled by ${s.cancelledBy}` : "Student Registration Cancelled",
  ...
  if (s.dropped && s.droppedAt) {
    allItems.push({
      title: s.droppedBy ? `Student Dropped from Program by ${s.droppedBy}` : "Student Dropped from Program",
  ...
  if (s.misStatus === "approved" && s.misApprovedAt) {
    allItems.push({
      title: s.misApprovedBy ? `MIS Approved by ${s.misApprovedBy}` : "MIS Approved",
  ...
  if (s.onboardingSubmitted) {
    const onboardingEntry = {
      title: s.onboardingSubmittedBy ? `Onboarding by ${s.onboardingSubmittedBy}` : "Onboarding",
  ...
  if (s.orientationCompleted) {
    const orientationEntry = {
      title: s.orientationCompletedBy ? `Orientation by ${s.orientationCompletedBy}` : "Orientation",
