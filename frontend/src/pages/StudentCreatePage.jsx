  ...
  const handleSave = async (arg) => {
    ...
    try {
      ...
      if (createdStudent?.id) {
        ...
        const updated = await updateStudent(createdStudent.id, payload);
  ...
  return (
    <div className="min-h-[calc(100vh-2rem)] flex flex-col">
      ...
      <div className="flex-1 space-y-4 pb-24">
        {createdStudent && (
          <p className="text-center text-lg font-semibold text-skillit">Successfully Added</p>
        )}
          ...
          <form onSubmit={handleSave}>
            <div className="grid gap-4 md:grid-cols-3">
              <Field label="Student Name" required>
                <Input value={form.customerName} onChange={updateField("customerName")} placeholder="Enter full name" required />
              </Field>
              <Field label="Parent Name" required>
                <Input value={form.primaryContactName} onChange={updateField("primaryContactName")} placeholder="Contact person name" required />
              </Field>
              <Field label="Student Email" required>
                <Input value={form.email} onChange={updateField("email")} type="email" placeholder="example@email.com" required />
              </Field>
              <Field label="Student Phone" required>
                <PhoneInput value={form.contactNumber} onChange={updateField("contactNumber")} placeholder="00000 00000" required />
              </Field>
              <Field label="Alternative Number" required>
                <PhoneInput value={form.altContactNumber} onChange={updateField("altContactNumber")} placeholder="00000 00000" required />
              </Field>
              <Field label="Batch" required>
                <Select value={form.batch} onChange={updateField("batch")} required>
              ...
              <Field label="Program" required>
                <Select value={form.program} onChange={updateField("program")} required>
                  <option value="" disabled>
                    Select program
                  </option>
              ...
              <Field label="Course Fee" required>
                <Input value={form.courseFee} onChange={updateField("courseFee")} type="number" min="0" placeholder="0" required />
              </Field>
            </div>
            ...
            <div className="mt-6 flex justify-end">
              ...
              disabled={!createdStudent ? !canCreateStudent : !canUpdateStudent}
              >
                {createdStudent ? "Edit Student" : "Add Student"}
              </Button>
            </div>
          </form>
        </section>
